...
import { api, errorMessage, type CreateIntentResponse, type IntentRequest, type RegisterVerifyResponse } from "./lib/api";
...
type State =
  ...
  | { step: "approve"; userId: string; pending: CreateIntentResponse; phase: "ready" | "prompting" | "verifying"; error: string | null }
  | { step: "certificate"; userId: string; certificate: SignedIntentCertificate };
  ...
  if (preview === "certificate") {
    ...
    return {
      ...
      certificate: {
        ...
        signature: "MEUCIQDq4v7k2Z0mF0f9Yw7Q8yG5r1X3Kp2sT4uV6wX8yZ0aBQIgC1dE2fG3hI4jK5lM6nO7pQ8rS9tU0vW1xY2zA3bC4dE",
        authenticatorData: "SZYN5YgOjGh0NBcPZHZgW4_krrmihjLHmVzzuoMdl2MFAAAAAQ",
        clientDataJSON: "eyJ0eXBlIjoid2ViYXV0aG4uZ2V0IiwiY2hhbGxlbmdlIjoiTkRabFpERTRabVZsTm1JM09XTXlZall6WVRZMVpUWTFZbUZrTURobFpUaGlOakEzWmpRelpqVTVaalJtTURjMllUSmhNRGRtTkdZeE5UVTFOekprTUEiLCJvcmlnaW4iOiJodHRwOi8vbG9jYWxob3N0OjUxNzMiLCJjcm9zc09yaWdpbiI6ZmFsc2V9",
...
export default function App() {
  ...
  const [health, setHealth] = useState<{ rpID: string; expectedOrigin: string } | null | "down">(null);

  useEffect(() => {
    api.health().then(setHealth).catch(() => setHealth("down"));
  }, []);

  const onRegistered = useCallback((username: string, _r: RegisterVerifyResponse) => {
    // Small pause so the success card is visible before moving on.
    setTimeout(() => setState({ step: "intent", userId: username, busy: false, error: null }), 900);
  }, []);
  ...
  async function createIntent(req: IntentRequest) {
    ...
    try {
      ...
      setState({ step: "approve", userId: state.userId, pending, phase: "ready", error: null });
    } catch (e) {
      setState({ step: "intent", userId: state.userId, busy: false, error: errorMessage(e) });
    }
  }
  ...
  async function approve() {
    ...
    try {
      ...
      setState({ step: "approve", userId, pending, phase: "verifying", error: null });
      const { certificate } = await api.attestIntent(pending.intentId, assertion);
      ...
      if (cancelled && state.phase !== "verifying") {
        ...
        setState({ step: "approve", userId, pending, phase: "ready", error: msg });
      } else {
        setState({ step: "intent", userId, busy: false, error: `${msg} — please re-submit the intent to get a fresh challenge.` });
      }
    }
  }
  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-4xl flex-col px-4 py-8 sm:px-6">
      <header className="mb-8 flex flex-wrap items-center justify-between gap-4">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-semibold tracking-tight">Cryptographic Intent Attestation</h1>
          </div>
          <p className="mt-1 text-xs text-slate-500">
            Sign what you mean, not what the page shows. RFC 8785 · SHA-256 · WebAuthn.
          </p>
        </div>
      ...
      <main className="flex-1">
        {state.step === "register" && <RegisterStep onRegistered={onRegistered} />}
        {state.step === "intent" && (
          <IntentStep
            userId={state.userId}
            busy={state.busy}
        ...
        {state.step === "approve" && (
          <ApproveCard
            ...
            onApprove={approve}
            onClearError={() => setState({ ...state, error: null })}
            onBack={() => setState({ step: "intent", userId: state.userId, busy: false, error: null })}
          />
        )}
